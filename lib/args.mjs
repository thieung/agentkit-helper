export const COMMANDS = new Set([
  "install", "update", "self-update", "update-all", "sync", "export", "doctor", "help",
]);
...
export function isDetectUpdate(options) {
  return options.command === "update" &&
    !options.all &&
    !options.binaryOnly &&
    !options.project &&
    !options.global &&
    !options.sshTarget &&
    !(options.target || options.runtime);
}
...
export function validateForCommand(options) {
  ...
  if (options.command === "doctor") {
    if (
      ...
      options.binaryOnly || options.all || options.dryRun || options.noSave || options.allowDowngrade ||
      hasDiscoveryOptions
    ) {
  ...
  if (options.command === "update") {
    if (options.all) {
      if (
        options.project || options.global || selectedTarget || options.kit || options.out ||
        options.binaryOnly || options.noSave || options.allowDowngrade
      ) {
    ...
    if (hasDiscoveryOptions) throw new Error("deep scan options are valid only with update --all");
    if (isDetectUpdate(options) && (options.kit || options.noSave)) {
      throw new Error("update without a scope detects cwd and global installs; use --project, --global, --runtime, or --all");
    }
    return;
  }
  if (options.command === "self-update") {
    if (
      ...
      options.noSave || options.all || hasDiscoveryOptions
    ) {
  ...
  if (options.command === "sync") {
    if (
      options.project || options.global || selectedTarget || options.kit || options.channel || options.out ||
      options.binaryOnly || options.all || options.noSave || options.allowDowngrade || hasDiscoveryOptions
    ) {
      throw new Error("sync infers channel from the installed ak binary and accepts only --dry-run and shared output flags");
    }
    return;
  }
  if (options.command === "export") {
    if (options.project || options.binaryOnly || options.all || options.noSave || options.runtime || options.allowDowngrade) {
...
export function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (argument) {
      ...
      case "--ssh":
      case "--vps":
        options.sshTarget = takeValue(argv, index, argument);
        options.global = true;
        index += 1;
        break;
      case "--identity":
        options.identityFile = takeValue(argv, index, argument);
        index += 1;
        break;
      case "--target":
...
function validateArgs(options) {
  ...
  if (options.sshTarget) {
    if (options.command && options.command !== "install" && options.command !== "update") {
      throw new Error("--ssh is valid only with install and update");
    }
    if (specHasProfileTarget(options.target || options.runtime)) {
      throw new Error("--ssh does not support custom profile targets");
    }
  }
  if (options.identityFile) {
    options.identityFile = String(options.identityFile).trim();
    if (!options.identityFile) {
      throw new Error("--identity requires a path");
    }
    if (!options.sshTarget) {
      throw new Error("--identity is valid only with --ssh");
    }
  }
  ...
  if (options.all && options.command && options.command !== "update" && options.command !== "update-all") {
    throw new Error("--all is valid only with update");
  }
  if (options.all && (options.project || options.global || options.target || options.runtime || options.binaryOnly)) {
    throw new Error("--all cannot be combined with project, global, target, or binary-only");
  }
