...
import { sshInvocationArgs } from "./commands.mjs";
...
export function runCapture(
  binary,
  args,
  { cwd = process.cwd(), stdio = ["ignore", "pipe", "pipe"], envOverrides = {}, envUnset = [] } = {},
) {
  return new Promise((resolve, reject) => {
    const invocation = spawnInvocation(binary, args);
    const child = spawn(invocation.binary, invocation.args, {
...
export function resolveSshBinary() {
  return process.env.AK_HELPER_SSH_BIN || "ssh";
}
...
export function runSsh(
  ...
  const args = sshInvocationArgs(host, remoteScript, { batchMode, identityFile, passwordOnly });
  return run(binary, args, { cwd, stdio, envOverrides, envUnset });
}

export function runSshCapture(
  host,
  remoteScript,
  {
    cwd = process.cwd(),
  ...
  const binary = resolveSshBinary();
  const args = sshInvocationArgs(host, remoteScript, { batchMode, identityFile, passwordOnly });
  const stdio = batchMode
    ? ["ignore", "pipe", "pipe"]
    : [process.stdin.isTTY ? "inherit" : "ignore", "pipe", process.stderr.isTTY ? "inherit" : "pipe"];
  return runCapture(binary, args, { cwd, stdio, envOverrides, envUnset });
}
export async function bootstrapRemoteAk(host, {
  batchMode = false,
  identityFile = null,
  passwordOnly = false,
} = {}) {
  const installCmd = "curl -fsSL https://agentkit.best/install.sh | sh";
  await runSsh(host, installCmd, { batchMode, identityFile, passwordOnly, stdio: "inherit" });
}
