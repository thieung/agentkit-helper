...
export function sshInvocationArgs(host, remoteScript, {
  batchMode = false,
  identityFile = null,
  passwordOnly = false,
} = {}) {
  const trimmedIdentity = typeof identityFile === "string" ? identityFile.trim() : "";
  if (trimmedIdentity && passwordOnly) {
    throw new Error("identityFile and passwordOnly cannot be used together");
  }
  const fullScript = `${REMOTE_PATH_EXPORT}; ${remoteScript}`;
  const remoteCommand = `bash -lc ${quote(fullScript)}`;
  const args = [];
  if (trimmedIdentity) {
    args.push("-i", trimmedIdentity, "-o", "IdentitiesOnly=yes");
  }
  if (passwordOnly) {
    args.push(
...
export function formatSshCommand(host, remoteScript, {
  batchMode = false,
  identityFile = null,
  passwordOnly = false,
  sshBinary = "ssh",
