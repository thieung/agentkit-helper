...
const messages = {
  en: {
    ...
    syncAction: "Sync ak binary and detected Kits",
    ...
    vpsScope: "Remote VPS via SSH",
    vpsHostPrompt: "SSH host (e.g. user@1.2.3.4 or alias)",
    vpsProbing: "Probing remote VPS ({host})…",
    vpsProbed: "Connected to remote VPS ({host}).",
    vpsAkMissing: "The ak CLI is not installed on remote host {host}.",
    vpsBootstrapPrompt: "Install ak on {host} via official installer?",
    vpsBootstrapping: "Installing ak on remote VPS…",
    vpsBootstrapped: "ak successfully installed on {host}.",
    vpsRecentHosts: "Choose a recent host or enter a new one:",
    vpsEnterNewHost: "+ Enter new host…",
    vpsIdentityPrompt: "How should SSH authenticate to this VPS?",
    vpsIdentityDefault: "Use SSH default (agent/config)",
    vpsIdentityPassword: "Password",
    vpsIdentityEnterPath: "+ Enter identity path…",
    vpsIdentityPathPrompt: "Path to SSH identity file",
    vpsIdentityNoneFound: "No SSH identity files were detected in ~/.ssh.",
    scopePrompt: "Where should AgentKit manage this Kit?",
    unsafeCwd: "The current directory ({cwd}) is not used as project scope.",
    linkedNativeDestination: "ak cannot write {path} because it is a symlink. Replace that symlink with a regular file, or remove it, then retry. AgentKit refuses linked native destinations.",
    ...
    noGlobalKitInstalls: "No AgentKit-owned {kit} global installs were detected. Install a Kit with --global first.",
    globalTargetNotInstalled: "{kit} is not installed globally for {target}. Installed: {installed}.",
    noneInstalled: "none",
    ...
    syncChannel: "Using installed ak channel: {channel}",
    syncBinaryCurrent: "ak binary already current: {version} ({channel})",
    syncBinaryUpdate: "ak binary update: {current} → {latest} ({channel})",
    syncBinarySkip: "ak binary will not change (status: {status})",
    syncHomeScope: "Current directory is home or filesystem root; syncing the binary and every globally installed runtime.",
    syncNoProject: "Current directory is not an AgentKit-owned project; project Kits are skipped.",
    syncProjectScope: "Current project Kits:",
    confirmSync: "Apply this sync plan?",
    syncApplyPlan: "Sync apply plan:",
    syncComplete: "Sync complete. Restart the affected runtimes.",
    syncNothing: "Nothing to sync.",
    cancelledSync: "Cancelled; sync was not applied.",
    syncBinaryDryRunBlock: "Dry run stops Kit previews because they need the newer binary. No changes were applied.",
  ...
  vi: {
    ...
    syncAction: "Sync ak binary và Kit đã detect",
    ...
    vpsScope: "Quản lý Kit trên VPS qua SSH",
    vpsHostPrompt: "Địa chỉ SSH (vd: user@1.2.3.4 hoặc alias)",
    vpsProbing: "Đang kiểm tra kết nối VPS ({host})…",
    vpsProbed: "Đã kết nối tới VPS ({host}).",
    vpsAkMissing: "CLI ak chưa được cài đặt trên máy chủ {host}.",
    vpsBootstrapPrompt: "Cài đặt ak trên {host} qua script chính thức?",
    vpsBootstrapping: "Đang cài đặt ak trên VPS…",
    vpsBootstrapped: "Đã cài đặt ak thành công trên {host}.",
    vpsRecentHosts: "Chọn host gần đây hoặc nhập host mới:",
    vpsEnterNewHost: "+ Nhập host mới…",
    vpsIdentityPrompt: "SSH sẽ xác thực với VPS này bằng cách nào?",
    vpsIdentityDefault: "Dùng SSH mặc định (agent/config)",
    vpsIdentityPassword: "Mật khẩu",
    vpsIdentityEnterPath: "+ Nhập đường dẫn identity…",
    vpsIdentityPathPrompt: "Đường dẫn file identity SSH",
    vpsIdentityNoneFound: "Không detect được identity file trong ~/.ssh.",
    scopePrompt: "AgentKit sẽ quản lý Kit này ở scope nào?",
    unsafeCwd: "Thư mục hiện tại ({cwd}) không được dùng làm project scope.",
    linkedNativeDestination: "ak không ghi được {path} vì đó là symlink. Hãy thay symlink bằng file thường, hoặc xóa file đó, rồi chạy lại. AgentKit từ chối native destination dạng linked.",
    ...
    noGlobalKitInstalls: "Không detect được {kit} global do AgentKit sở hữu. Hãy cài Kit với --global trước.",
    globalTargetNotInstalled: "{kit} chưa được cài global cho {target}. Đang có: {installed}.",
    noneInstalled: "không có",
    ...
    syncInventory: "Danh sách sync đã scan:",
    syncChannel: "Dùng channel của ak đang cài: {channel}",
    syncBinaryCurrent: "ak binary đã là bản hiện tại: {version} ({channel})",
    syncBinaryUpdate: "Cập nhật ak binary: {current} → {latest} ({channel})",
    syncBinarySkip: "ak binary không đổi (status: {status})",
    syncHomeScope: "Thư mục hiện tại là home hoặc filesystem root; sync binary và mọi runtime đã cài global.",
    syncNoProject: "Thư mục hiện tại không phải project do AgentKit sở hữu; bỏ qua Kit của project.",
    syncProjectScope: "Kit của project hiện tại:",
    confirmSync: "Áp dụng kế hoạch sync này?",
    syncApplyPlan: "Kế hoạch áp dụng sync:",
    syncComplete: "Sync hoàn tất. Hãy khởi động lại runtime liên quan.",
    syncNothing: "Không có gì để sync.",
    cancelledSync: "Đã hủy; chưa áp dụng sync.",
    syncBinaryDryRunBlock: "Dry run dừng Kit preview vì cần binary mới hơn. Chưa có thay đổi nào được áp dụng.",
